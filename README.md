...
Run each pipeline as a complete independent experiment from Cycle 0 to Cycle 5.
...
```
The separate `old_pipeline/` and `new_pipeline/` directories are intentional. They preserve the files and outputs from both implementations and make it possible to present or inspect each experiment independently.
...
For every model/pipeline combination:

```text
1. Verify split.json
2. Initialize Active + Random Cycle-0 states
3. Train Baseline
4. Run Active C1-C5
5. Run Random C1-C5
6. Evaluate Baseline
7. Evaluate Active C5
8. Evaluate Random C5
9. Optionally run integrity audit
10. Optionally compare per-image predictions
11. Optionally generate training graphs
```

The four experiment families are:
...
ViT    + new pipeline
```
Finish one branch from Cycle 0 through Cycle 5 before treating another branch as a new experiment.

For example:

ResNet old pipeline:
C0 -> C1 -> C2 -> C3 -> C4 -> C5 -> evaluate

ResNet new pipeline:
fresh C0 -> C1 -> C2 -> C3 -> C4 -> C5 -> evaluate
...
```powershell
python src/resnet/old_pipeline/init_resnet_state.py
```

For an isolated new-pipeline experiment:

```powershell
python src/resnet/new_pipeline/init_resnet_state.py
```

If the initializer is shared:

```powershell
python src/resnet/init_resnet_state.py
```
...
```powershell
python src/resnet/old_pipeline/train.py
```

or, if named explicitly:

```powershell
python src/resnet/old_pipeline/train_baseline.py
```

For the new pipeline:

```powershell
python src/resnet/new_pipeline/train.py
```
...
```powershell
python src/resnet/old_pipeline/selection.py --cycle 1 --checkpoint checkpoints/resnet/old_pipeline/baseline/resnet50_baseline_repeat_new.pt
```
...
```powershell
python src/resnet/old_pipeline/active_learning.py --cycle 1
```

Train:

```powershell
python src/resnet/old_pipeline/train.py --cycle 1
```
...
```powershell
python src/resnet/old_pipeline/active_learning.py --cycle 2
python src/resnet/old_pipeline/train.py --cycle 2
```
...
```powershell
python src/resnet/old_pipeline/active_learning.py --cycle 3
python src/resnet/old_pipeline/train.py --cycle 3
```
...
```powershell
python src/resnet/old_pipeline/active_learning.py --cycle 4
python src/resnet/old_pipeline/train.py --cycle 4
```
...
```powershell
python src/resnet/old_pipeline/active_learning.py --cycle 5
python src/resnet/old_pipeline/train.py --cycle 5
```
...
```powershell
python src/resnet/new_pipeline/active_learning.py --cycle 1
python src/resnet/new_pipeline/train.py --cycle 1
```
...
python src/resnet/new_pipeline/active_learning.py --cycle 2
python src/resnet/new_pipeline/train.py --cycle 2
```
...
python src/resnet/new_pipeline/active_learning.py --cycle 3
python src/resnet/new_pipeline/train.py --cycle 3
```
...
python src/resnet/new_pipeline/active_learning.py --cycle 4
python src/resnet/new_pipeline/train.py --cycle 4
```
...
python src/resnet/new_pipeline/active_learning.py --cycle 5
python src/resnet/new_pipeline/train.py --cycle 5
```
...
```powershell
python src/resnet/old_pipeline/random_selection.py --cycle 1
```
...
```powershell
python src/resnet/old_pipeline/update_random_state.py --cycle 1
python src/resnet/old_pipeline/train_random.py --cycle 1
```

Repeat for cycles 2-5:

```powershell
python src/resnet/old_pipeline/random_selection.py --cycle 2
python src/resnet/old_pipeline/update_random_state.py --cycle 2
python src/resnet/old_pipeline/train_random.py --cycle 2
```
...
```powershell
python src/vit/old_pipeline/init_vit_state.py
```

or:

```powershell
python src/vit/new_pipeline/init_vit_state.py
```
...
```powershell
python src/vit/old_pipeline/train_baseline_vit_ft2.py --epochs 10
```

New pipeline:

```powershell
python src/vit/new_pipeline/train_baseline_vit_ft2.py --epochs 10
```

Use the same epoch budget for Baseline, Active, and Random within the same controlled comparison.
...
```powershell
python src/vit/old_pipeline/active_learning_vit.py --cycle 1
python src/vit/old_pipeline/train_vit_ft2.py --cycle 1 --epochs 10
```
...
python src/vit/old_pipeline/active_learning_vit.py --cycle 2
python src/vit/old_pipeline/train_vit_ft2.py --cycle 2 --epochs 10
```
...
```powershell
python src/vit/new_pipeline/active_learning_vit.py --cycle 1
python src/vit/new_pipeline/train_vit_ft2.py --cycle 1 --epochs 10
```
...
python src/vit/new_pipeline/active_learning_vit.py --cycle 2
python src/vit/new_pipeline/train_vit_ft2.py --cycle 2 --epochs 10
```
...
```powershell
python src/vit/old_pipeline/random_selection_vit.py --cycle 1
```
...
```powershell
python src/vit/old_pipeline/update_random_state_vit.py --cycle 1
python src/vit/old_pipeline/train_random_vit_ft2.py --cycle 1 --epochs 10
```

Repeat for Cycles 2-5.

Use the corresponding `new_pipeline/` paths if Random is kept independently for the new experiment.
...
```powershell
python src/resnet/old_pipeline/evaluate.py --checkpoint checkpoints/resnet/old_pipeline/baseline/best.pt --split test --name baseline_test

python src/resnet/old_pipeline/evaluate.py --checkpoint checkpoints/resnet/old_pipeline/active/cycle_05.pt --split test --name active_c5_test

python src/resnet/old_pipeline/evaluate.py --checkpoint checkpoints/resnet/old_pipeline/random/cycle_05.pt --split test --name random_c5_test
```
...
```powershell
python src/vit/old_pipeline/evaluate_vit.py --checkpoint checkpoints/vit/old_pipeline/baseline/best.pt --split test --name vit_baseline_test

python src/vit/old_pipeline/evaluate_vit.py --checkpoint checkpoints/vit/old_pipeline/active/cycle_05.pt --split test --name vit_active_c5_test

python src/vit/old_pipeline/evaluate_vit.py --checkpoint checkpoints/vit/old_pipeline/random/cycle_05.pt --split test --name vit_random_c5_test
```
...
If a run must be restarted:

1. identify the exact model/pipeline branch;
2. archive any outputs that should be preserved;
3. remove only that run's generated states, selections, checkpoints, logs, and results;
4. rerun Cycle-0 initialization;
5. retrain the baseline;
6. rerun acquisition from Cycle 1.
...
---
#   a c t i v e _ l e a r n i n g _ p i p e l i n e  
 