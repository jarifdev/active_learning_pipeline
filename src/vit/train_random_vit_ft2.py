"""
train_random_vit_ft2.py

Partial-fine-tuning version of the RANDOM-control ViT trainer.

Reuses the EXISTING states/vit/random/cycle_XX.json so that the selected
training images are unchanged. This isolates the effect of partial
fine-tuning from the effect of data selection.

Outputs go to:
    checkpoints/vit_ft2/random/
    logs/vit_ft2/random/
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Use exactly the same epoch budget as baseline and active.",
    )
    args = parser.parse_args()

    STATES_DIR = PROJECT_ROOT / "states" / "vit" / args.pipeline / "random"
    CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints" / "vit" / args.pipeline / "random"
    LOG_DIR = PROJECT_ROOT / "logs" / "vit" / args.pipeline / "random"
    ...
    print("\n" + "=" * 65)
    print(f"VIT PARTIAL FINE-TUNING - RANDOM CYCLE {args.cycle}")
    print("=" * 65)
