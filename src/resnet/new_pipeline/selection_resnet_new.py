"""
selection_resnet_new.py

Improved active-learning selection for the ResNet50 experiment.
...
Strategy per cycle:
    ...
    7. Apply k-center greedy to the ResNet50 embeddings of those top 40.
...
Outputs remain compatible with active_learning.py:
...
Additional audit output:
    selections/cycle_XX_pool_scores.csv

Example:
    python src/selection_resnet_new.py \
        --cycle 1 \
        --checkpoint checkpoints/resnet50_baseline_repeat.pt
"""
...
STATES_DIR = PROJECT_ROOT / "states" / "resnet" / "new_pipeline" / "active"
SELECTIONS_DIR = PROJECT_ROOT / "selections" / "resnet" / "new_pipeline" / "active"
...
def build_model_for_checkpoint(device):
    """Rebuild standard torchvision ResNet50 with a 2-class classifier."""
...
def load_checkpoint(path, device):
    """Load a ResNet50 checkpoint without downloading pretrained weights."""
...
def infer_with_embeddings(model, dataset, device, batch_size=16):
    """
    Run inference and capture the 2048-D representation entering model.fc.
    Returns probabilities, embeddings, and paths in dataset order.
    """
    loader = DataLoader(
        dataset,
    ...
    handle = model.fc.register_forward_pre_hook(feature_pre_hook)
...
def infer_training_embeddings(model, dataset, device, batch_size=16):
    """Extract ResNet50 embeddings for the fixed Cycle-0 labeled reference set."""
    loader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        required=True,
        help="ResNet50 checkpoint from the previous cycle.",
    )
    parser.add_argument(
        "--candidate-size",
        type=int,
    ...
    print(f"\nResNet50 Cycle {args.cycle} - improved active selection")
